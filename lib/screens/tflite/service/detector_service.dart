...
import 'package:digipad_flutter/screens/tflite/utils/non_max_suppression.dart';
import 'package:flutter/foundation.dart';
import 'package:flutter/services.dart';
import 'package:image/image.dart' as image_lib;
import 'package:digipad_flutter/screens/tflite/models/recognition.dart';
import 'package:digipad_flutter/screens/tflite/utils/image_utils.dart';
...
class Detector {
  static const String _modelPath = 'assets/models/model2.tflite';
  ...
  static Future<Interpreter> _loadModel() async {
    ...
    // NO agregamos el delegado XNNPack para evitar el error de compatibilidad.
    if (Platform.isAndroid) {
      interpreterOptions.addDelegate(XNNPackDelegate());
    }

    // La aplicación ahora usará la implementación de CPU estándar, que es más compatible.
    return Interpreter.fromAsset(_modelPath, options: interpreterOptions);
  }
...
class _DetectorServer {
  ...
  static const int mlModelInputSize = 480;
  ...
  Map<String, dynamic> analyseImage(
    ...
    final imageInput = image_lib.copyResize(
      image!,
      width: mlModelInputSize, // 480
      height: mlModelInputSize, // 480
    );
    ...
    // Post-procesamiento con NMS
    List<Rect> boxes = [];
    List<double> scores = [];
    List<int> classIds = [];

    // Transponemos la matriz para facilitar el acceso: de [6, 4725] a [4725, 6]
    for (int i = 0; i < predictions[0].length; i++) {
      final score = predictions[4][i];

      if (score > confidence) {
        scores.add(score);
        classIds.add(predictions[5][i].toInt());
        ...
        // Las coordenadas están normalizadas (0-1), las escalamos al tamaño de la entrada
        final left = (x - w / 2) * mlModelInputSize;
        final top = (y - h / 2) * mlModelInputSize;
        final right = (x + w / 2) * mlModelInputSize;
        final bottom = (y + h / 2) * mlModelInputSize;
        boxes.add(Rect.fromLTRB(left, top, right, bottom));
      }
    }

    // Aplicamos NMS para obtener los índices de las cajas a conservar
    final List<int> nmsIndices = nonMaxSuppression(
      boxes,
      scores,
      iouThreshold: 0.4,
    );
  ...
  // En lib/screens/tflite/service/detector_service.dart

  List<List<double>> _runInference(List<List<List<num>>> imageMatrix) {
    // Set input tensor [1, 480, 480, 3]
    final input = [imageMatrix];

    // Set output tensor for a single output model.
    final output = [List.filled(6, List.filled(4725, 0.0))];

    // ¡USA el método run(), no runForMultipleInputs()!
    _interpreter!.run(input, output);

    // La función analyseImage espera una List<List<Object>>,
    // así que devolvemos la salida directamente.
    return output.first;
  }
}
